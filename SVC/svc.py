...
# RMVPE F0 extractor 
def load_rmvpe(path_model: str | Path,
               device: torch.device):
...
# Convert an Hz-scale F0 sequence into the discrete bin representation expected by length_regulator DiT. Values outside [f0_min, f0_max] mapped to 0.
def rmvpe_to_coarse_f0(f0_hz: np.ndarray,
                       n_bins: int = 256,
                       f0_min: float = 50.,
                       f0_max: float = 1100.):
    # log-linear binning 
    f0_log = np.log2(f0_hz)
    f0_log_min = np.log2(f0_min)
    f0_log_max = np.log2(f0_max)

    bins = (f0_log - f0_log_min) / (f0_log_max - f0_log_min) * (n_bins - 2) + 1
    bins = np.clip(np.round(bins), 1, n_bins - 1)
    bins[np.isnan(f0_hz) | (f0_hz < 1e-6)] = 0
    return bins.astype(np.int64)


# Key-shift 
def transpose_f0(f0_hz: np.ndarray, semitone_shift: float):
    factor = 2 ** (semitone_shift / 12.0)
    return f0_hz * factor


# Map F0 in Hz → 256-bin coarse representation , any value <= 0 is mapped to 0.
def f0_to_coarse(f0_hz: np.ndarray | torch.Tensor) -> np.ndarray:
    if isinstance(f0_hz, torch.Tensor):
        f0_hz = f0_hz.cpu().numpy()
    f0_hz = f0_hz.clip(min=1e-5)

    # MIDI-like mapping 
    midi = 69 + 12 * np.log2(f0_hz / 440.0)
    coarse = np.round((midi - 32) * 256 / (110 - 32)).astype(np.int32)
    coarse = np.clip(coarse, 1, 255)            # 0 reserved for unvoiced
    coarse[f0_hz < 1e-4] = 0                    # unvoiced frames
    return coarse
...
if __name__ == "__main__":
    ...
    dit_config_path =os.path.join(path_vc_model,'config_dit_mel_seed_uvit_whisper_base_f0_44k.yml')
    config = yaml.safe_load(open(dit_config_path, "r"))
    
    # read voice conversion model
    dit_checkpoint_path = os.path.join(path_vc_model,'DiT_seed_v2_uvit_whisper_base_f0_44k_bigvgan_pruned_ft_ema.pth')
    ...
    # read RMVPE F0 extractor
    path_rmvpe   = 'models/F0/rmvpe.pt' 
    rmvpe_model  = load_rmvpe(path_rmvpe, device)
    ...
    inference_cfg_rate = 0.7 # External conditions ratio
    ...
    sr = config["preprocess_params"]["sr"] # 44k
    ...
    source = "examples/source/C_u_again.wav"
    target = "examples/reference/s1p2.wav"
    source_audio,fs = librosa.load(source, sr=sr) # Read the source and target voices and resample them to sr, 44k
    ...
    with torch.no_grad(),torch.inference_mode():
        ...
        ref_target_lengths = torch.LongTensor([ref_mel.size(2)]).to(ref_mel.device)
          ...
          # F0 extraction
        # rmvpe expects raw PCM at the trained sample rate(16 kHz here)
        f0_ref_hz = rmvpe_model.infer_from_audio(
            ref_waves_16k.squeeze(0).cpu().numpy()
        )
        # same for the source chunk
        f0_src_hz = rmvpe_model.infer_from_audio(
            src_waves_16k.squeeze(0).cpu().numpy()
        )

        # transpose the source so its key matches target.
        # For singing voice we usually offset so medians match:
        semitone_shift = 12 * np.log2(np.median(f0_ref_hz[f0_ref_hz>0]) /
                                      np.median(f0_src_hz[f0_src_hz>0]))
        f0_src_hz_shifted = transpose_f0(f0_src_hz, semitone_shift)

        # Pass raw Hz to length_regulator (it will quantize internally)
        F0_ori = torch.from_numpy(f0_ref_hz).unsqueeze(0).to(device)
        F0_alt = torch.from_numpy(f0_src_hz_shifted).unsqueeze(0).to(device)


        # Adjust the length to make the length of token feature and mel consistent
        cond, _, _, _, _ = inference_module.length_regulator(S_alt, ylens=src_target_lengths, n_quantizers=3, f0=F0_alt)
        ...
        while processed_frames < cond.size(1):
            ...
            with torch.autocast(device_type=device.type, dtype=torch.float16 if fp16 else torch.float32):
                ...
                vc_target = inference_module.cfm.inference(cat_condition,
                                                        ...
                                                        ref_mel, style, F0_alt, diffusion_steps,
